...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_profile'])) {
    ...
    try {
        ...
        $signer_update_data = ['name' => $name, 'profile_picture' => $new_pic_filename];
        ...
        header("Location: dashboard.php?msg=" . urlencode(" Profile updated!") . "&type=success");
        exit;
    } catch (Exception $e) {
        $conn->rollBack();
...
foreach ($raw_notifications as $r) {
    ...
    if ($status === 'Pending') {
        $msg = "<strong>NEW REQUEST:</strong> Student {$r['student_name']} submitted a clearance request on {$date}.";
        $class = 'approved';
        $type = 'new_req';
