...
$org_query = "SELECT signer_id, name, requirements, profile_picture FROM signer WHERE signer_type = 'Organization' ORDER BY name";
...
$faculty_query = "
    ...
    ORDER BY
        FIELD(position, 'Dean', 'Department Head', 'Adviser', 'SA Coordinator', 'Registrar', 'Librarian'), name";
...
foreach ($all_faculty_members as $member) {
    ...
    if ($member['position'] === 'Adviser' && $member['signer_id'] == $adviser_id) { $include = true; }
...
function getSignerStatus($type, $ref_id, $signatures) {
    foreach ($signatures as $sig) {
        ...
        if ($sig['signer_type'] == $type && $sig['signer_id'] == $ref_id) {
       ...
       $sign_order = 10;
        if ($signer_type === 'Faculty') {
            foreach ($faculty_members as $member) {
                ...
                if ($member['signer_id'] == $signer_ref_id) {
                    $pos = $member['position'] ?? '';
        ...
        $ok = $clearanceObj->submitSignatureUpload($clearance_id, $signer_ref_id, $uploaded_file, $sign_order);
...
$faculty_query = "SELECT signer_id, name, position, requirements, department_id, profile_picture
    FROM signer WHERE signer_type = 'Faculty' AND (requirements IS NOT NULL OR position IN ('Dean', 'SA Coordinator', 'Adviser', 'Department Head', 'Registrar', 'Librarian'))
    ORDER BY FIELD(position, 'Dean', 'Department Head', 'Adviser', 'SA Coordinator', 'Registrar', 'Librarian'), name";
    ...
    <div class="page-content-wrapper">
        ...
        <div class="signer-grid">
            <?php foreach ($org_result as $org):
                ...
                $status = getSignerStatus('Organization', $org['signer_id'], $currentSignatures);
                
                $org_name = htmlspecialchars($org['name']);
                ...
                $onclick_handler = $is_disabled ? '' : "openModal('Organization',{$org['signer_id']}, '{$js_safe_req}', '".addslashes($org_name)."')";
            ?>
            ...
            <?php foreach ($faculty_members as $faculty):
                ...
                $status = getSignerStatus('Faculty', $faculty['signer_id'], $currentSignatures);
                
                $display_name = htmlspecialchars($faculty['name']);
                ...
                $onclick_handler = $is_disabled ? '' : "openModal('Faculty',{$faculty['signer_id']}, '{$js_safe_req}', '".addslashes($display_name)."')";
            ?>
                <div class="<?= $card_class ?>" onclick="<?= $onclick_handler ?>" style="<?= $is_disabled ? 'cursor: default; opacity: 0.6;' : 'cursor: pointer;' ?>">
                    <div class="logo-small" style="background-image: url('<?= htmlspecialchars($logo_url) ?>');"></div>
                    <h3><?= $display_name ?></h3>
