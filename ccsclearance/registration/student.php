...
$dept_id = $course = $year_level = $section = "";
$confirm_password = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $adviser_stmt = $conn->prepare("
        SELECT signer_id
        FROM signer
        WHERE signer_type = 'Faculty' AND position = 'Adviser'
        LIMIT 1
    ");
    $adviser_stmt->execute();
    $adviser_id = $adviser_stmt->fetchColumn();

    if (empty($school_id) || empty($fName) || empty($lName) || empty($email) || empty($password) || empty($confirm_password) || empty($dept_id) || empty($course) || empty($year_level) || empty($section)) {
        ...
        $result = $accountObj->register($fName, $mName, $lName, $dept_id, null, null, $course, $year_level, $section, $adviser_id);
        ...
        if ($result === true) {
           ...
           $school_id = $fName = $mName = $lName = $email = $confirm_password = $dept_id = $course = $year_level = $section = "";
