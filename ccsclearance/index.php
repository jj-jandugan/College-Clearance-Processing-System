                ...
                if (strpos($user['email'], '@clearance.local') !== false) {
                    ...
                    switch ($user['role']) {
                        case 'student':
                            ...
                            break;
                        case 'signer':
                            // Get signer type to determine redirect
                            require_once "classes/Signer.php";
                            $signer = new Signer();
                            $signerDetails = $signer->getSignerDetails($accountObj->ref_id);
                            $_SESSION['signer_type'] = $signerDetails['signer_type'] ?? 'Faculty';

                            if ($signerDetails['signer_type'] == 'Faculty') {
