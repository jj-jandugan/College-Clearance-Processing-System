...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_dept') {
    ...
    } elseif ($action === 'provision_admin_faculty') {
        ...
        if (empty($fName) || empty($lName) || empty($position) || empty($temp_password)) {
            $message = "❌ Name, Position, and Temporary Password are required.";
            $messageType = "error";
        } else {
            $result = $adminObj->provisionFaculty($fName, $mName, $lName, null, $position, $temp_password);
            if ($result === true) {
                $full_name = trim("$fName $mName $lName");
                $message = "✅ Administrative Faculty '{$full_name}' ({$position}) successfully added! Temp Password: " . htmlspecialchars($temp_password);
                $messageType = "success";
            } else {
                $message = "❌ Failed to provision: $result";
                $messageType = "error";
            }
        }
    }
...
<body>
        ...
        <div class="page-content-wrapper">
            ...
            <div class="container" style="margin-top: 25px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                    ...
                    <button class="btn-primary" onclick="openAddAdminFacultyModal()" style="margin: 0;">+ Add Administrative Faculty</button>
                </div>
                <table>
                    <thead>
                        <tr>
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <form method="POST" action="manage_departments.php" style="display:inline;" onsubmit="return confirm('Confirm action for <?= htmlspecialchars($sf['name']) ?>?');">
    ...
    <div id="addAdminFacultyModal" class="modal" style="display: none;">
        <div class="modal-content" style="max-width: 500px;">
            <span class="close" onclick="closeModal('addAdminFacultyModal')">&times;</span>
            <h3 style="margin-top: 5px;">Add Administrative Faculty</h3>
            <p style="font-size: 0.9em; color: #555; margin-bottom: 15px;">Phase 1 Setup: Enter details & temp password. User sets email on first login.</p>
            <form method="POST" action="manage_departments.php">
                <input type="hidden" name="admin_action" value="provision_admin_faculty">

                <div style="display: flex; gap: 10px; margin-top: 15px;">
                    <div class="form-group" style="flex: 1;"><label>First Name:</label><input type="text" name="new_fName" required></div>
                    <div class="form-group" style="flex: 1;"><label>Last Name:</label><input type="text" name="new_lName" required></div>
                </div>
                <div class="form-group"><label>Middle Name (Optional):</label><input type="text" name="new_mName" placeholder="Optional"></div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="new_temp_password" placeholder="e.g., Password123" required>
                </div>

                <div class="modal-actions" style="justify-content: flex-end; margin-top: 20px;">
                    <button type="button" onclick="closeModal('addAdminFacultyModal')" class="btn-primary" style="background-color: grey;">Cancel</button>
...
function openAddAdminFacultyModal() {
    document.getElementById('addAdminFacultyModal').style.display = 'flex';
}
