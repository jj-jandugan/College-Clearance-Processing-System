...
$latest_period_query = "SELECT school_year, term, remarks
                        FROM clearance
                        WHERE school_year IS NOT NULL AND term IS NOT NULL
                        ORDER BY clearance_id DESC LIMIT 1";
$latest_period = $conn->query($latest_period_query)->fetch(PDO::FETCH_ASSOC);

if ($latest_period) {
    
    $active_check_query = "SELECT COUNT(*) as pending_count
                          FROM clearance
                          WHERE school_year = :sy
                          AND term = :term
                          AND status = 'Pending'";
    $stmt_active = $conn->prepare($active_check_query);
    $stmt_active->execute([
        ':sy' => $latest_period['school_year'],
        ':term' => $latest_period['term']
    ]);
    $active_result = $stmt_active->fetch(PDO::FETCH_ASSOC);

    $is_cycle_active = $active_result['pending_count'] > 0;
    $current_cycle_name = htmlspecialchars($latest_period['school_year']) . ' ' . htmlspecialchars($latest_period['term']);

    
    $cycle_id_query = "SELECT clearance_id FROM clearance
                       WHERE school_year = :sy AND term = :term
                       ORDER BY clearance_id DESC LIMIT 1";
    $stmt_cid = $conn->prepare($cycle_id_query);
    $stmt_cid->execute([':sy' => $latest_period['school_year'], ':term' => $latest_period['term']]);
    $current_cycle_id = $stmt_cid->fetchColumn();
} else {
    $is_cycle_active = false;
    $current_cycle_name = 'None Active';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'start_cycle') {
        ...
        if (empty($school_year) || empty($term)) {
            ...
            $recent_cycle_query = "SELECT school_year, term FROM clearance
                                  WHERE school_year IS NOT NULL AND term IS NOT NULL
                                  ORDER BY clearance_id DESC LIMIT 1";
            $recent_cycle = $conn->query($recent_cycle_query)->fetch(PDO::FETCH_ASSOC);

            if ($recent_cycle) {
                
                $recent_year = $recent_cycle['school_year'];
                $recent_term = $recent_cycle['term'];
                $new_year = $school_year;
                $new_term = $term;

                
                $term_order = ['1st' => 1, '2nd' => 2, 'Summer' => 3];
                $recent_term_order = $term_order[$recent_term] ?? 0;
                $new_term_order = $term_order[$new_term] ?? 0;

                
                $is_past_cycle = false;
                if ($new_year < $recent_year) {
                    $is_past_cycle = true;
                } elseif ($new_year == $recent_year && $new_term_order <= $recent_term_order) {
                    $is_past_cycle = true;
                }

                if ($is_past_cycle) {
                    $message = "❌ Cannot create a cycle in the past! Most recent cycle: {$recent_year} {$recent_term}. New cycles must be chronologically after this.";
                    $messageType = "error";
                    goto skip_cycle_start;
                }
            }
    ...
    elseif ($action === 'stop_cycle' && $is_cycle_active && $current_cycle_id) {
        try {
            ...
            $period_data_query = "SELECT school_year, term FROM clearance WHERE clearance_id = :cid";
            $period_stmt = $conn->prepare($period_data_query);
            $period_stmt->execute([':cid' => $current_cycle_id]);
            $period_data = $period_stmt->fetch(PDO::FETCH_ASSOC);

            $update_master = "UPDATE clearance
                             SET remarks = CONCAT(COALESCE(remarks, ''), ' | CYCLE_STOPPED_BY_ADMIN'),
                                 date_completed = COALESCE(date_completed, NOW())
                             WHERE school_year = :sy AND term = :term";
            $stmt = $conn->prepare($update_master);
            $stmt->execute([
                ':sy' => $period_data['school_year'],
                ':term' => $period_data['term']
            ]);
...
<body>
    <div class="sidebar">
        ...
        <a href="manage_clearance.php" class="active">Manage Clearance</a>
                        ...
                        <?php else: ?>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($record['course']) . ' ' . htmlspecialchars($record['section']) ?></td>
