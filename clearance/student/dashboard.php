...
require_once "../classes/Account.php"; // Ensure Account.php is included
...
// --- FIX: Initialize objects for use in POST and fetch blocks ---
$student_id = $_SESSION['ref_id'];
$clearanceObj = new Clearance();
$accountObj = new Account();

// --- Connect to DB for transactions and raw queries ---
$db = new Database();
$conn = $db->connect();

// --- NEW: Initial Profile Data Fetch ---
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_profile'])) {
    ...
    // Handle Image Upload (Simplified logic for filename update)
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] == UPLOAD_ERR_OK) {
        $target_dir = "../assets/img/profiles/"; // Assuming a dedicated folder
        ...
        // Delete old file if a new one was uploaded and it's not the default
    ...
    try {
        ...
        // 1. Update Student Table (Name, Picture)
        $student_update_data = [
            'fName' => $fName,
            'mName' => $mName,
            'lName' => $lName,
            'profile_picture' => $new_pic_filename // Save the new filename
        ];
        // FIX: Pass the transactional connection $conn
        $clearanceObj->updateStudentProfile($student_id, $student_update_data, $conn);

        // 2. Update Account Table (Email, Password)
        // If password is empty, pass null to the update function to skip password update
        // FIX: Pass the transactional connection $conn
...
// --- FIX: Correctly construct and display student name (F. [M.I.] L) with default fallback ---
...
// Construct the name: First Name + Middle Initial (if available) + Last Name
...
// --- FIX: ROBUST IMAGE PATH LOGIC ---
// 1. Get filename from DB. If empty/null, force 'profile.png'
$profile_pic_name = !empty($profile_data['profile_picture']) ? $profile_data['profile_picture'] : 'profile.png';

// 2. Determine path.
// If it starts with 'student_', it's a custom upload in /assets/img/profiles/
// Otherwise, it's a default image in /assets/img/
if (strpos($profile_pic_name, 'student_') === 0) {
    $profile_pic_path = '../assets/img/profiles/' . $profile_pic_name;
} else {
    // Falls back here if name is 'profile.png' or any other default
    $profile_pic_path = '../assets/img/profile.png';
}
// ------------------------------------------------------------------------

// --- EXISTING POST HANDLERS (Notification) ---
...
if (isset($_POST['mark_read_id']) && isset($_POST['mark_read_type']) && !isset($_POST['delete_one'])) {
    ...
    $redirect_link = $_POST['redirect_link'] ?? 'dashboard.php';
    header("Location: " . $redirect_link);
    exit;
}
// --- END EXISTING POST HANDLERS ---
...
<style>
    /* --- HEADER & NOTIFICATION STYLES (Copied from provided snippet for completeness) --- */
...
<div class="main-content">
    ...
    <div class="page-content-wrapper">
        ...
        <div class="card-container">
            ...
            <div class="card approved" onclick="window.location.href='dashboard.php'" style="cursor: pointer;">
                <h3>Approved Request</h3>
                <p><?= $summary['Approved'] ?? 0 ?></p>
            </div>

            <div class="card rejected" onclick="window.location.href='dashboard.php'" style="cursor: pointer;">
                <h3>Rejected</h3>
                <p><?= $summary['Rejected'] ?? 0 ?></p>
            </div>

            <div class="card cancelled" onclick="window.location.href='dashboard.php'" style="cursor: pointer;">
                <h3>Cancelled</h3>
            ...
            <table>
                ...
                <?php if (!empty($recent_requests)): ?>
                    <?php foreach ($recent_requests as $r):
                        $display_status = htmlspecialchars($r['signer_status'] ?? $r['status']);
...
<div id="profileModalStudent" class="modal" style="display:none;">
    <div class="modal-content-profile">
        <span class="close" onclick="closeModal('profileModalStudent')">&times;</span>
        ...
        <form method="POST" action="dashboard.php" enctype="multipart/form-data">
            ...
            <div class="modal-actions" style="justify-content: flex-end; margin-top: 20px;">
                <button type="button" onclick="closeModal('profileModalStudent')" class="cancel-modal-btn">Cancel</button>
...
<script>
    ...
    function openProfileModal(role) {
        if (role === 'student') {
            // Ensure the PHP variables holding the current state are read into JS before showing the modal
            ...
            // REMOVED: const currentContact = '<?= htmlspecialchars(addslashes($profile_data['contact_number'] ?? '')) ?>';
            ...
            // 1. Update form fields with freshest PHP data
            ...
            // REMOVED: document.getElementById('contact_number').value = currentContact;
            document.getElementById('email').value = currentEmail;

            // 2. Update image preview and hidden input
    ...
    // Assign handler to profile name on page load
    document.addEventListener('DOMContentLoaded', () => {
        const profileNameElement = document.querySelector('.sidebar-profile .profile-name');
        if (profileNameElement) {
            profileNameElement.onclick = () => openProfileModal('student');
        }
    });
